        ...
        if (!err) {
          ...
          login(name, password)
            ...
            .catch((err) => {
              ...
              self.logging = false;
              self.error = getError(err.error);
            });
        }
      });
    },
    afterLogin(res) {
      setting.systemName = this.systemName
      this.logging = false;
      const loginRes = res.data;

      const { user, permissions, roles } = loginRes.result;
      this.setUser(user);
